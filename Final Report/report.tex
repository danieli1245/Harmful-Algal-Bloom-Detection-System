...
\IfFileExists{production_architecture.png}{%
  ...
  \fbox{\parbox{\columnwidth}{\centering Placeholder for system\_architecture.png}}%
}
\caption{System architecture illustrating the flow from frontend user interactions to backend services, external data access, and machine learning model inference.}
...
The Prediction Service operationalizes spatiotemporal ML: it sources satellite data (e.g., chlorophyll-a, SST, RRS), preprocesses modalities into a spatiotemporal datacube for a target area and time window, and dynamically selects the model based on the user's tier: a lightweight XGBoost baseline (Free Tier), a CNN--LSTM model (Tier~1), and a CNN--BiLSTM with attention (Tier~2). For image uploads, a YOLO model performs object detection.
...
\subsection{Free Tier: XGBoost Baseline}
...
\subsection{Tier 1: CNN--LSTM Spatio-Temporal Model}

\subsubsection{Rationale and Design}
Tier~1 preserves spatial adjacency and temporal dynamics by applying shared convolutional blocks to each day in the 10-day sequence using TimeDistributed Conv2D layers. These convolutional layers extract spatial features from daily satellite images while maintaining spatial relationships. The extracted features are then spatially pooled and flattened before being passed sequentially to an LSTM layer that captures temporal dependencies over the 10-day period.

This design addresses the limitations of Free Tier, which flattens data prematurely losing spatial structure, and enables the model to learn patterns of bloom formation, intensity gradients, and temporal persistence critical for effective HAB detection.

\begin{figure}[!t]
    \centering
    \includegraphics[width=0.3\textwidth]{Tier1_Model.png}
    \caption{Block diagram of the Tier~1 CNN--LSTM spatio-temporal network architecture.}
    \label{fig:tier1_model_architecture}
\end{figure}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.25\textwidth]{Tier2_Model.png}
    \caption{Block diagram of the Tier~2 CNN--BiLSTM--Attention model architecture.}
...
YOLOv11-large, implemented with the Ultralytics framework, accurately detects and localizes HAB regions by generating bounding boxes with confidence scores, supporting critical environmental monitoring applications. The architecture includes a CSP (Cross Stage Partial) backbone for efficient feature extraction, a PANet (Path Aggregation Network) neck for multi-scale fusion, and decoupled anchor-free classification and regression heads. SiLU (Sigmoid Linear Unit) activations enable multi-scale predictions and real-time inference.

The model was trained for 30 epochs on images resized to $640 \times 640$ pixels with batch size 16, using the standard YOLO loss combining classification, localization, and objectness components. Given input images $I \in \mathbb{R}^{H \times W \times 3}$, the model outputs bounding boxes $B_i$, confidence scores $c_i$, and class probabilities $p_i$. Predictions are filtered by a confidence threshold $\theta$ and refined with non-maximum suppression at an IoU threshold of 0.45.

Validation results show 84.7\% precision, 43.7\% recall, 48.9\% mAP@0.5, 29.5\% mAP@0.5--0.95, and a fitness score of 0.315, reflecting high detection confidence with moderate recall. YOLOv11-large powers premium-tier image-based prediction services, automating preprocessing, detection, and visualization of HAB regions with confidence scores and spatial localization.
...
The principal technical contributions of this work are founded upon architectural decisions designed to ensure system scalability and reliability:

\begin{itemize}
\item \textbf{Serverless Machine Learning Deployment:} Advanced spatiotemporal models were operationalized within a fully serverless environment, converting research prototypes into production-ready, on-demand AI services suitable for data-intensive applications.
\item \textbf{Asynchronous Task Orchestration:} FastAPI’s asynchronous capabilities were utilized in the User Service Gateway, enabling efficient handling of long-running prediction tasks and maintaining system responsiveness for concurrent users.
\item \textbf{Dynamic Tiered Model Routing:} The system employs dynamic routing of requests to different machine learning models (e.g., geospatial and YOLO architectures) according to user subscription tier, achieving scalable service differentiation in terms of accuracy and computational resources.
\item \textbf{End-to-End CI/CD Automation:} A fully automated continuous integration and deployment pipeline was implemented using Google Cloud Build, facilitating rapid, reliable deployments directly from source repositories without manual intervention.
\end{itemize}

\subsection{Project Repositories}
The source code supporting this work is publicly available in three GitHub repositories corresponding to the \href{https://github.com/DV821/hab-frontend}{frontend}, \href{https://github.com/sagar-rsh/hab-backend}{user service backend}, and \href{https://github.com/danieli1245/Harmful-Algal-Bloom-Detection-System}{prediction service backend}.
...
Our HAB Detection System successfully achieves the transformation from reactive harmful algal bloom management to a proactive approach that we were aiming for by using satellite data and machine learning. The system achieves its core objective of providing HAB predictions for 10 days ahead with a 94\% accuracy, achieving a clear improvement over traditional 3 day forecasts with 60\% accuracy. This achievement gives the important 5-7 days needed for an effective response, potentially preventing significant economic losses - with the 2018 Florida red tide causing \$2.7B in tourism losses alone, our 10-day prediction provides the critical 5-7 day advance warning window that emergency management protocols required for an effective response.  

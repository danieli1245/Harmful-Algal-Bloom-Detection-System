...
The system evolved from an MVP into a robust, scalable microservices platform organized as three decoupled services: a Next.js frontend, a FastAPI-based User Service (gateway), and an internal FastAPI Prediction Service. Decoupling isolates user-facing interactions from heavy model execution, enabling resilience and independent scaling. The logical interaction of these components is shown in Figure~\ref{fig:logical_architecture}.

\begin{figure}[H]
\centering
\IfFileExists{Images/logical_architecture.png}{%
  \includegraphics[width=\columnwidth]{Images/logical_architecture.png}%
}{%
  \fbox{\parbox{\columnwidth}{\centering Placeholder for logical\_architecture.png}}%
}
\caption{High-level logical system architecture with decoupled services and responsibilities.}
...
\subsection{Data Flow and Service Integration}
...
\begin{enumerate}
\item A logged-in user initiates prediction via map coordinate selection or image upload
\item The frontend sends an HTTPS request with a JWT token to the User Service
\item The User Service validates authentication and checks subscription tier via Firestore
\item Requests are forwarded internally to the Prediction Service with API-key authentication, selecting the appropriate model based on user tier
\item The Prediction Service executes the corresponding model (XGBoost, CNN-LSTM, CNN-BiLSTM-Attention, or YOLOv11l) and compiles results
\item Results propagate back through the User Service to the frontend
\item The frontend renders predictions through maps or image annotations
\end{enumerate}
...
\begin{tikzpicture}[
    node distance=0.3cm,
    layer/.style={rectangle, draw=black, fill=blue!15, minimum width=3cm, minimum height=0.5cm, align=center, font=\footnotesize},
    input/.style={rectangle, draw=black, fill=green!15, minimum width=3cm, minimum height=0.5cm, align=center, font=\footnotesize},
    output/.style={rectangle, draw=black, fill=red!15, minimum width=3cm, minimum height=0.5cm, align=center, font=\footnotesize}
]
\node[input] (input) {Input: $(10,64,64,3)$};
\node[layer, below=of input] (conv1) {TimeDistributed Conv2D (32, $3\times3$)};
\node[layer, below=of conv1] (bn1) {TimeDistributed BatchNorm};
\node[layer, below=of bn1] (pool1) {TimeDistributed MaxPool2D ($2\times2$)};
\node[layer, below=of pool1] (conv2) {TimeDistributed Conv2D (64, $3\times3$)};
\node[layer, below=of conv2] (bn2) {TimeDistributed BatchNorm};
\node[layer, below=of bn2] (pool2) {TimeDistributed MaxPool2D ($2\times2$)};
\node[layer, below=of pool2] (flatten) {TimeDistributed Flatten};
\node[layer, below=of flatten] (lstm) {LSTM (128 units)};
\node[layer, below=of lstm] (dropout1) {Dropout (0.4)};
\node[layer, below=of dropout1] (dense1) {Dense (64, ReLU)};
\node[output, below=of dense1] (output) {Dense (2, Softmax)};
\draw[->, thick] (input) -- (conv1);
\draw[->, thick] (conv1) -- (bn1);
\draw[->, thick] (bn1) -- (pool1);
\draw[->, thick] (pool1) -- (conv2);
\draw[->, thick] (conv2) -- (bn2);
\draw[->, thick] (bn2) -- (pool2);
\draw[->, thick] (pool2) -- (flatten);
\draw[->, thick] (flatten) -- (lstm);
\draw[->, thick] (lstm) -- (dropout1);
\draw[->, thick] (dropout1) -- (dense1);
\draw[->, thick] (dense1) -- (output);
\end{tikzpicture}
\caption{Tier~2 CNN--LSTM spatio-temporal model.}
\label{fig:tier2_architecture}
\end{figure}
...
\node[input] (input) {Input: $(10,64,64,3)$};
\node[layer, below=of input] (conv1) {TimeDistributed Conv2D (32, $3\times3$)};
\node[layer, below=of conv1] (pool1) {TimeDistributed MaxPool2D ($2\times2$)};
\node[layer, below=of pool1] (flatten) {TimeDistributed Flatten};
\node[layer, below=of flatten] (bilstm) {BiLSTM (128 units, return\_sequences)};
\node[layer, below=of bilstm] (attention) {Attention Layer};
\node[layer, below=of attention] (dense1) {Dense (128, ReLU)};
\node[layer, below=of dense1] (dropout) {Dropout (0.5)};
\node[output, below=of dropout] (output) {Dense (2, Softmax)};
\draw[->, thick] (input) -- (conv1);
\draw[->, thick] (conv1) -- (pool1);
\draw[->, thick] (pool1) -- (flatten);
\draw[->, thick] (flatten) -- (bilstm);
\draw[->, thick] (bilstm) -- (attention);
\draw[->, thick] (attention) -- (dense1);
\draw[->, thick] (dense1) -- (dropout);
\draw[->, thick] (dropout) -- (output);
\end{tikzpicture}
\caption{Tier~3 CNN--BiLSTM with attention.}
\label{fig:tier3_architecture}
\end{figure}
...
The YOLOv11l architecture employs a three-component design:
\begin{itemize}
...
Validation metrics achieved 84.7\% precision, 43.7\% recall, 48.9\% mAP@0.5, 29.5\% mAP@0.5--0.95, and fitness score of 0.315, indicating high confidence in predicted detections with moderate coverage of true HAB regions.

\subsubsection{Integration and Service}

The YOLOv11l model serves premium tiers for image-based predictions, processing uploaded images through automatic preprocessing, object detection, and visualization of detected HAB regions with confidence scores and spatial localization.
